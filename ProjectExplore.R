...
# Survival curves for race_new
survfit2(Surv(survyears, statusca) ~ race_labeled, data = Ovarian.ex) %>% 
  ggsurvfit() +
  labs(
    x = "Years",
...
fit1 <- coxph(Surv(survyears, statusca) ~ race_labeled_releved, data = Ovarian.ex)
summary(fit1)
...
x = Ovarian.ex %>% 
  select(ses, urban_new, marital, histology_new, stage, grade, site, age_cat, primsurgery, radiation, chemo, primary, microconfirm, dxtotreat)

rmst2(Ovarian.ex$survyears, Ovarian.ex$statusca, Ovarian.ex$race_new1, tau=10)
rmst2(Ovarian.ex$survyears, Ovarian.ex$statusca, Ovarian.ex$race_labeled_releved, tau=10, covariates=x)


# Decision Tree
LTRCART.obj <- LTRCART(Surv(survyears, statusca) ~ ses+urban_new +
                         marital +histology_new +stage+grade+site+age_cat+
                         primsurgery+radiation+chemo+primary+microconfirm+
                         race_labeled_releved+dxtotreat, Ovarian.without0s)
